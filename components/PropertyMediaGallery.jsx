// bizhub Property detail page  Media section
"use client";
...
export default function PropertyMediaGallery({ images = [] }) {
  ...
  // --- Layout logic (curbsite-style: main image + 2 stacked thumbnails) ---
  const hasTwoCol = total >= 2;
  ...
  return (
    <div className="relative">
      {/* Mobile: single image, curbsite-style with photo count badge */}
      <div className="sm:hidden relative aspect-[4/3] rounded-lg overflow-hidden bg-gray-100">
        <div
          className="absolute inset-0 w-full h-full cursor-pointer"
          onClick={() => openModal(0)}
        >
          {validImages[0] && !brokenIndices.has(0) ? (
            <img
              src={validImages[0]}
              alt="Property main"
        ...
        {total > 1 && (
          <button
            ...
            className="absolute bottom-3 right-3 inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/90 text-[13px] font-medium text-gray-800 shadow-sm border border-gray-300"
          >
            <Grid size={14} />
            {total} photos
          </button>
      ...
      {/* Desktop: curbsite-style split grid — main image full height + 2 stacked thumbnails */}
      <div
        className="hidden sm:grid gap-1.5"
        style={{
          height: "520px",
          gridTemplateColumns: hasTwoCol ? "1fr 1fr" : "1fr",
          gridTemplateRows: "1fr 1fr",
        }}
      >
        {/* Main image — left column, full height */}
        <button
          type="button"
          onClick={() => openModal(0)}
        ...
        >
          {gridImages0Valid(validImages, brokenIndices) ? (
            <img
              src={validImages[0]}
              alt="Property main"
              onError={() => markBroken(0)}
        ...
        {total >= 2 && (
          ...
          >
            {validImages[1] && !brokenIndices.has(1) ? (
              <img
                src={validImages[1]}
                alt="Property 2"
        ...
        {total >= 3 && (
          ...
          >
            {validImages[2] && !brokenIndices.has(2) ? (
              <img
                src={validImages[2]}
                alt="Property 3"
                onError={() => markBroken(2)}
                className="absolute inset-0 w-full h-full object-cover"
              />
            ) : (
              <div className="absolute inset-0 w-full h-full flex flex-col items-center justify-center text-gray-300 gap-1.5">
      ...
      {/* "Show all photos" button — always visible on desktop */}
      <button
        type="button"
        onClick={() => openModal(0)}
        className="hidden sm:inline-flex absolute bottom-4 right-4 items-center gap-2 px-4 py-2 rounded-full bg-white text-gray-800 text-xs sm:text-sm font-semibold shadow border border-gray-400 hover:bg-gray-200 transition-colors z-10"
      >
        <Grid size={14} />
...
function gridImages0Valid(validImages, brokenIndices) {
  return Boolean(validImages[0]) && !brokenIndices.has(0);
}
